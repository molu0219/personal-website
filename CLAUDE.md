...
- [ ] 對應 TODO 任務已標 `[x]`
- [ ] PRD 該功能驗收條件已打勾
- [ ] 若有下一個 Agent 應該知道的經驗（技術選擇、踩坑、困難解法）→ DECISION.md 已新增記錄
...
**DECISION.md 寫入判斷**：問自己「這件事是下一個 Agent 應該知道的嗎？」

寫：做了什麼技術選擇（為什麼選 A 不選 B）、遇到什麼困難（怎麼解決的）、踩了什麼坑（避免重蹈覆轍）、引入新依賴的原因、刻意不做某事的理由。
